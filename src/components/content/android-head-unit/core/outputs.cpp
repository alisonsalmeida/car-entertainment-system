...
VideoOutput::VideoOutput(DesktopEventCallbacks* callbacks) : callbacks(callbacks) {
    GstBus *bus;
    g_hu = callbacks->aa_service->g_hu;
    GError *error = NULL;
    ...
    const char* vid_launch_str = "appsrc name=mysrc is-live=true block=false max-latency=100000 do-timestamp=true stream-type=stream typefind=true ! "
                                 ...
                                 "h264parse ! "
                                 "avdec_h264 ! "
                                 "videoscale name=myscale ! "
                                 "videoconvert name=myconvert ! "
                                 "xvimagesink name=mysink force-aspect-ratio=false";

    vid_pipeline = gst_parse_launch(vid_launch_str, &error);

    bus = gst_pipeline_get_bus(GST_PIPELINE(vid_pipeline));
    gst_bus_add_watch(bus, (GstBusFunc) bus_callback, &callbacks->aa_service->gst_app);
    ...
    gst_video_overlay_set_window_handle(sink, callbacks->aa_service->widget->winId());
...
void VideoOutput::ChangeSizeRender(unsigned int x, unsigned int y, unsigned int width, unsigned int height)
{
    if(vid_pipeline) {
        GstVideoOverlay* sink = GST_VIDEO_OVERLAY(gst_bin_get_by_name(GST_BIN(vid_pipeline), "mysink"));
        gst_video_overlay_set_render_rectangle(sink, x, y, width, height);
...
void VideoOutput::SetStatePlayer(bool play) {
    if (vid_pipeline) {
        gst_element_set_state(vid_pipeline, play == true ? GST_STATE_PLAYING : GST_STATE_PAUSED);
    }
}
